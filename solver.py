    ...
    def train(self):
        for iter_time in range(0, self.flags.iters+1, self.flags.train_interval):
            ...
            for iter_ in range(1, self.flags.train_interval+1):
                x_imgs, y_imgs = self.dataset.train_next_batch(batch_size=self.flags.batch_size)
                if iter_ > 0:
                    g_loss = self.model.train_gen(x_imgs, y_imgs);self.print_info(iter_time + iter_, 'g_loss', g_loss)
                for i in xrange(self.disc_iters):   #disc_iters
                    d_loss = self.model.train_dis(x_imgs, y_imgs);self.print_info(iter_time + iter_, 'd_loss', d_loss)
            ############################################################################################
